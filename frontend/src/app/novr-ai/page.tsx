...
const CONTEXT_STATS = [
    ...
    { label: 'Top MITRE Tactic', value: 'Initial Access', color: 'text-green' },
];
    ...
    return (
        <div className="space-y-2 text-xs text-foreground leading-relaxed">
            {lines.map((line, i) => {
                ...
                if (line.startsWith('- ') || line.startsWith('• ')) return <p key={i} className="flex items-start gap-2"><span className="text-green flex-shrink-0">•</span><span>{line.replace(/^[-•]\s/, '')}</span></p>;
                if (/^\d+\./.test(line)) return <p key={i} className="flex items-start gap-2"><span className="text-green font-bold flex-shrink-0">{line.match(/^\d+/)?.[0]}.</span><span>{line.replace(/^\d+\.\s/, '')}</span></p>;
                    ...
                    <div className="flex-1 overflow-y-auto p-4 scrollbar-thin space-y-4">
                        {messages.length === 0 && (
                            <div className="flex flex-col items-center justify-center h-full text-center">
                                <div className="w-14 h-14 rounded-2xl bg-green from-green to-green flex items-center justify-center mb-4 shadow-lg">
                                ...
                                <div className="flex flex-col gap-2 w-full max-w-md">
                                    {SUGGESTED_PROMPTS.map(p => (
                                        <button key={p} onClick={() => send(p)}
                                            className="text-left text-xs text-foreground bg-card-muted hover:bg-card-muted border border-border hover:border-green/30 rounded-lg px-4 py-2.5 transition-all">
                                            {p}
                                        </button>
                                    ))}
                                </div>
                        ...
                        {messages.map((m, i) => (
                            <div key={i} className={`flex gap-3 ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
                                <div className={`w-7 h-7 rounded-lg flex items-center justify-center text-[11px] font-black flex-shrink-0 ${m.role === 'user' ? 'bg-amber' : 'bg-green from-green to-green'}`}>
                                    {m.role === 'user' ? 'U' : '🤖'}
                                </div>
                                <div className={`max-w-[75%] rounded-xl px-4 py-3 ${m.role === 'user' ? 'bg-green/10 border border-green/30 text-foreground' : 'bg-card-muted border border-border'}`}>
                        ...
                        {loading && (
                            <div className="flex gap-3">
                                <div className="w-7 h-7 rounded-lg bg-green from-green to-green flex items-center justify-center text-[11px] flex-shrink-0">🤖</div>
                    ...
                    <div className="border-t border-border p-3">
                        <div className="flex gap-2 items-end">
                            <textarea
                                ...
                                rows={2}
                                className="flex-1 bg-card-muted border border-border rounded-xl px-4 py-2.5 text-xs text-foreground placeholder:text-foreground-muted focus:outline-none focus:ring-2 focus:ring-green/20 resize-none"
                            />
